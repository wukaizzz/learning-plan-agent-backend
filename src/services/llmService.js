...
const DEEPSEEK_BASE_URL = process.env.DEEPSEEK_API_URL?.replace('/chat/completions', '') || 'https://api.deepseek.com';
...
/**
 * 使用 V3 对话模型 + withStructuredOutput 输出结构化 JSON
 * 自动通过 Zod schema 校验
 *
 * @param {string} prompt - 提示词
 ...
 */
export async function structuredOutput(prompt, schema, { systemPrompt } = {}) {
  const model = getChatModel();
  const structuredModel = model.withStructuredOutput(schema, { name: 'structured_output' });
